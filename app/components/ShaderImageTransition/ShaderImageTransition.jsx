...
import styles from './ShaderImageTransition.module.css';

// Step 2 conversion: port `dist/script.js` into React.
// This file initializes a three.js scene, loads textures and applies a
// shader-based transition. The original CSS is still not converted in this
// step (kept for step 3). This component uses refs and dynamic imports and
...
const PIXELS = new Float32Array(
  [
    1, 1.5, 2, 2.5, 3, 1, 1.5, 2, 2.5, 3, 3.5, 4, 2, 2.5, 3, 3.5, 4, 4.5,
    5, 5.5, 6, 3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5, 9, 20, 100,
  ].map((v) => v / 100)
);

export default function ShaderImageTransition() {
  const containerRef = useRef(null);
  const thumbnailsBridgeRef = useRef(null); // will be set by effect to handle clicks
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        // expose thumbnail click handler to JSX via ref bridge
        thumbnailsBridgeRef.current = (idx) => {
          if (idx === activeImage || isAnimating) return;
          transitionImage = idx;
          progress = 0;
          isAnimating = true;
          // update active class on thumbnails is handled by React side
        };
    ...
    return () => {
      ...
      try {
        ...
        textures.forEach((t) => { try { t.dispose && t.dispose(); } catch (e) {} });
      } catch (e) {
        // ignore
      }
    };
  ...
  // Thumbnail click bridge - the effect populates `thumbnailsBridgeRef.current` with a handler
  const handleThumbClick = (idx) => {
    if (thumbnailsBridgeRef.current) thumbnailsBridgeRef.current(idx);
  };

  return (
    <div className={styles.root}>
      <div className={styles.containerGallary}>
        <div className={styles.borderOutside}>
          <div className={styles.canvasWrapper} ref={containerRef}>
            <span className={styles.borderInside} />
          </div>
        </div>
        <div className={styles.thumbnails}>
          {THUMBS.map((t, idx) => (
            ...
            >
              <img className={styles.thumbnailImg} src={t.url} alt={t.title} />
